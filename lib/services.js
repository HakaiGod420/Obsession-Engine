...
export function applyAIResults(context) {
    ...
    for (const rawName of charNames) {
        ...
        if (Array.isArray(charResult.hiddenThoughts)) {
            for (const thought of charResult.hiddenThoughts.slice(0, 3)) {
                const text = typeof thought === 'string' ? thought.trim() : '';
                if (text) addThought(context, matchedName, text);
            }
            profile.lastThoughtAt = new Date().toISOString();
        }

        addEventLogEntry(context, matchedName, 'ai_analysis',
            'AI analyzed recent chat: stats, goals, and thoughts updated'
        );
        applied++;
    }
