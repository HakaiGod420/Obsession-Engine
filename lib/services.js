...
function sanitizeGoalTitle(title) {
    if (typeof title !== 'string') return '';
    let t = title.trim();
    if (!t) return '';
    // Safety net: keep AI-generated goals from referring directly to the user placeholder or second-person pronouns.
    t = t.replace(/\{\{user\}\}/gi, 'them');
    t = t.replace(/\byour\b/gi, 'their');
    t = t.replace(/\byours\b/gi, 'theirs');
    ...
    for (const rawName of charNames) {
        ...
        if (charResult.goalProgress && typeof charResult.goalProgress === 'object') {
            for (const [goalId, progress] of Object.entries(charResult.goalProgress)) {
                ...
                if (goal && !goal.completed) {
                    const newProgress = Math.max(goal.progress, Math.round(Number(progress)) || 0);
                    updateGoal(context, matchedName, goalId, { progress: newProgress });
                }
            }
        }
        ...
        if (Array.isArray(charResult.newGoals)) {
            ...
            for (const title of charResult.newGoals.slice(0, 3)) {
                const trimmed = sanitizeGoalTitle(typeof title === 'string' ? title : '');
                if (!trimmed) continue;
...
function extractJson(text) {
    ...
    // Try to find properly balanced JSON object
    let depth = 0;
    let start = -1;
    let end = -1;
    ...
    let str = text.slice(start, end + 1);
    ...
    const repairs = [
        { name: 'malformed-placeholder', re: /"(\w+)":\s*\\?\([|a-zA-Z_0-9\s]*\|?/g, fn: '"$1": 0' },
        { name: 'unescaped-newline', re: /([^\\])("(?:[^"\\]|\\.)*\n)/g, fn: '$1\\n' },
...
export async function initializeCharacter(context, charName) {
    ...
    if (!context?.chatMetadata) {
        showToast('Error', 'Load a chat first before initializing characters. Character profiles are saved to chat metadata.', 'error');
        return null;
    }
    ...
    try {
        ...
        if (Array.isArray(parsed.suggestedGoals)) {
            ...
            if (profile) {
                ...
                for (const goalTitle of parsed.suggestedGoals.slice(0, 5)) {
                    const trimmed = sanitizeGoalTitle(goalTitle);
        ...
        if (Array.isArray(parsed.additionalCharacters)) {
            for (const extra of parsed.additionalCharacters) {
                ...
                if (Array.isArray(extra.suggestedGoals)) {
                    ...
                    if (eProfile) {
                        const existingTitles = new Set((eProfile.goals || []).map(g => g.title.toLowerCase()));
        ...
        const resultPayload = parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed : {};
        return { ...resultPayload, actualName: primaryName };
    } catch (err) {
