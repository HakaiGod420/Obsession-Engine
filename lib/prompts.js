...
- The intensity multiplier scales how aggressively you push stats. Higher intensity = more dramatic responses to events.`,
        userContent: `Current character states:\n\n${charBlock}\n\n---\n\nRecent conversation:\n\n${msgs}\n\n---\n\nAnalyze and return JSON with stat deltas, goal updates, and hidden thoughts. Only include characters that appeared. Only include fields that are relevant.`,
    };
}
...
Respond ONLY with valid JSON:
{
  ...
  "personality": {
    "craziness": number 0-10,
    "manipulation": number 0-10,
    "assertiveness": number 0-10,
    "submissiveness": number 0-10,
    "jealousy": number 0-10,
    "possessiveness": number 0-10
  },
  "tone": number -5 to 5,
  "intensityMultiplier": number 0.1-3.0,
  "darknessCeiling": number 0-10,
...
- LOVE: High if the character is affectionate, in love, possessive, devoted. 0 for strangers. 80+ for yandere/obsessed.
- LUST: High if the character has strong physical desire, breeding kinks, or sexual themes. Explicit character descriptions or sexual scenarios = 50+.
- HATE: High if the character is tsundere, rivals, enemies, conflicted. Usually 0 for pure love.
- TRUST: High if the character trusts user (old friends, partners). Low if strangers or betrayal themes.
- JEALOUSY: High if possessive, yandere, or explicitly jealous. Low for carefree/poly.
- SANITY: Low for crazy/yandere/broken (1-4). High for stable/wholesome (7-10).
- CRAZINESS: Scale based on obsessiveness described. Yandere = 7-10. Normal crush = 1-3.
- MANIPULATION: High for gaslighters, guilt-trippers, schemers. 0 for innocent/naive.
- ASSERTIVENESS: High for dominant/confident. Low for shy/submissive.
- SUBMISSIVENESS: High for submissive/devoted. Low for dominant/controlling.
- TONE: Positive (+3 to +5) for dark/violent/twisted themes. Negative (-5 to -3) for wholesome/consent-focused. 0 for neutral.
- DARKNESS CEILING: 8-10 for yandere/violence/mind-break. 3-5 for playful teasing. 0-2 for wholesome.
- INTENSITY: 2.0-3.0 for extreme obsession. 0.5-1.0 for mild. Default 1.0.
- GROWTH RATES: Higher for obsessive characters (love growth 2-4). Lower for slow-burn (0.2-0.5).
- Sanity growth should be negative (draining) for crazy chars, near 0 for stable.
