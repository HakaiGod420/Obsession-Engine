...
export function processMessage(context, lastMessage) {
    ...
    if (settings.connectionProfileId) {
        const thoughtCooldown = 20;
        const lastThoughtMsg = profile.lastThoughtMsg || 0;
        if (data.global.msgCount - lastThoughtMsg >= thoughtCooldown) {
            profile.lastThoughtMsg = data.global.msgCount;
            saveDynamicsData(context);
            generateHiddenThoughts(context, charName).then(text => {
                ...
                console.info(`[ObsessionEngine] Auto-generated ${lines.length} hidden thought(s) for ${charName}`);
            }).catch(err => {
                console.error('[ObsessionEngine] Auto hidden thoughts failed:', err);
            });
        }
    }
}
