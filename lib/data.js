...
export function renameCharProfile(context, oldName, newName) {
    if (!oldName || !newName || oldName === newName) return false;
    const data = getDynamicsData(context);
    if (!data.characters[oldName]) return false;
    if (data.characters[newName]) return false;
    data.characters[newName] = data.characters[oldName];
    data.characters[newName].name = newName;
...
export function addGoal(context, charName, goal) {
    ...
    if (activeGoals.length > 5) {
        activeGoals.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
        const toRemove = new Set(activeGoals.slice(0, activeGoals.length - 5).map(g => g.id));
        profile.goals = profile.goals.filter(g => !toRemove.has(g.id));
    }
...
export function addThought(context, charName, thought) {
    ...
    if (profile.thoughts.length > 3) profile.thoughts.length = 3;
    saveDynamicsData(context);
}

export function setThoughts(context, charName, texts) {
    const profile = getCharProfile(context, charName);
    if (!profile) return;
    const now = new Date().toISOString();
    profile.thoughts = (Array.isArray(texts) ? texts : [])
        .filter(t => typeof t === 'string' && t.trim())
        .slice(0, 3)
        .map(t => ({ id: generateId(), text: t.trim(), timestamp: now }));
    saveDynamicsData(context);
}
...
export function pruneCharactersToChat(context) {
    ...
    for (const name of Object.keys(data.characters)) {
        if (!chatNames.has(name) && !data.characters[name]?.initialized) {
